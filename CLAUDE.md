...
- Swap success: mutation `onSuccess` clears the amount, persists the notification (`addNotification` — `id`/`createdAt` reuse the Transaction's `id`/`timestamp`), fires the toast and `router.navigate('/')` (back to Portfolio so the updated balances are visible). Toast = transient half of req. 4, notification = persisted half.
- Notifications history: `NotificationsScreen` (`features/notifications/notifications-screen.tsx`) — virtualized `FlatList` (growable, unlike the fixed asset list) + memoized `NotificationRow` (primitive props; masks amounts + USD under `hideAmounts`); symbols resolved via `getAsset` at render, falling back to the raw id. Clear-all = native `headerRight` trash via `<Stack.Screen options>` , hidden when empty. Timestamps via `formatDateTime` (Intl `dateStyle: 'medium'` + `timeStyle: 'short'`, `features/shared/money.ts`).
